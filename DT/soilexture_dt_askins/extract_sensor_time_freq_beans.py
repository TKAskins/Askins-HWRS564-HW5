"""
extract_sensor_time_freq_beans.py
For beans decision trees, counts how many unique physical-sensor selections
(sensor type + depth + timestep, deduplicated per tree) fall into early
(day 0-2), middle (day 3-5), and late (day 6-7) timestep bins, broken
...
CROP = 'beans'
FLUX_COL, FLUX_DEPTH, FLUX_DIR = 'flux_60', '60', '60cm'

BINS = [('early (day 0-2)', 0, 2), ('middle (day 3-5)', 3, 5), ('late (day 6-7)', 6, 7)]
...
# ── catalogue (beans only, 36 trees = 3 objectives x 12 cost configs) ────────
...
csv_path = out_dir / 'table_beans_sensor_time_bins.csv'
df_out.to_csv(csv_path, index=False)
print(f'\n[saved -> report_tables/{csv_path.name}]')

json_path = out_dir / 'table_beans_sensor_time_bins.json'
with open(json_path, 'w') as f:
    json.dump({'beans': rows}, f, indent=2)
