...
# from cpython.pycapsule cimport py_print
...
cdef inline int find_max_in_2d_array(vector[vector[float64_t]]  arr) noexcept nogil:
    cdef int rows = arr.size()
    cdef int col = 0
    cdef int max_ind = 0
    cdef float64_t max_val = arr[0][0]
    cdef int i, j
...
cdef inline int node_split_best(
    ...
    # cdef float64_t best_cost_objective = -INFINITY
    # cdef float64_t current_cost_objective = -INFINITY
    ...
    # cdef float32_t threshold
    cdef intp_t feature
    
    cdef int32_t sensor_n
    cdef int32_t depth_n
    cdef int32_t time_n

    cdef int32_t sensor_f
    cdef int32_t depth_f
    cdef int32_t time_f

    cdef int max_ind
    cdef float64_t max_id
    ...
    # const int32_t size_n = n_features
    # cdef float64_t[size_n,3] obj_arr
    ...
    # cdef intp_t time_char = 't'
    # cdef intp_t depth_char = 'd'
    ...
    while (f_i > n_total_constants and  # Stop early if remaining features
        ...
        for i in range(n_searches):
            ...
            while p < end_non_missing:
                ...
                if current_proxy_improvement > best_proxy_improvement:
                    ...
                    current_split.improvement = current_proxy_improvement
        ...
        # printf("feature: %s\n", buffer)
        # py_print(feature)
        # sprintf(buffer, "%f",feature_values[p])
        # printf("threshold: %s\n", buffer)
        ...
        for node_id in range(node_count):
            ...
            # if (sensor_f == sensor_n) and (time_f == time_n) and depth_flag:
            #     time_flag = True
            #     current_cost += time_cost

            # if time_flag or depth_flag:
            #     current_cost += sensor_cost[sensor_f]
            ...
            # sprintf(buffer, "%f",current_cost)
            # printf("cost: %s\n", buffer)
            # sprintf(buffer, "%f",current_cost_objective)
            # printf("cost_objective: %s\n", buffer)
            # # printf("\n")

        best_split.cost = current_cost

        # current_cost_objective = best_split.improvement + current_cost
        # best_split.cost_objective = current_cost_objective

        ind = n_visited_features - 1

        obj_arr[ind][0] = best_split.improvement
        obj_arr[ind][1] = current_cost
    ...
    inplace_sort_2d_array_by_two_columns(obj_arr, 0, 1)

    max_ind = find_max_in_2d_array(obj_arr)
    # max_id = obj_arr[max_ind, 2]

    best_ind = 0

    n_ind = -1
    while n_ind < n_features - 1:
        n_ind += 1

        diff = fabs(obj_arr[max_ind][0] - obj_arr[n_ind][0]) / obj_arr[max_ind][0]

        if diff <= cost_threshold:
            best_ind = n_ind
            break

    # current_cost_objective = current_proxy_improvement + current_cost
    feature_ind = <int>obj_arr[best_ind][2]
    best_split = split_list[feature_ind]

    best_split.cost = obj_arr[best_ind][1]
    # best_split.improvement = obj_arr[best_ind,0]
    
    # if current_cost_objective > best_cost_objective:
        # best_proxy_improvement = current_proxy_improvement
        # best_cost_objective = current_cost_objective
        # current_split.cost = current_cost
        # current_split.cost_objective = current_cost_objective
        # best_split = current_split  # copy
