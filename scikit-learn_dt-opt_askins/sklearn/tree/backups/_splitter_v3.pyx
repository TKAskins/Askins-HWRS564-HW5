...
cdef void copy_splitrecord(SplitRecord* src, SplitRecord* dst) noexcept nogil:
...
cdef inline int node_split_best(
    ...
    cdef int r_ind, c_ind, n_ind, ind
    ...
    while (f_i > n_total_constants and  # Stop early if remaining features
        ...
        obj_arr[ind][0] = <float>best_split.improvement
        obj_arr[ind][1] = <float>best_split.cost
    ...
    if False:
        ...
        # current_split = split_list[feature_ind]
        # sprintf(buffer3, "%zd",current_split.feature)
        sprintf(buffer3, "%zd",min_ind)
        printf("%s,%s,%s\n", buffer, buffer2, buffer3)
        
        max_ind = find_max_in_2d_array(obj_arr,0)
        sprintf(buffer, "%f",obj_arr[max_ind][0])
        sprintf(buffer2, "%f",obj_arr[max_ind][1])        
        sprintf(buffer3, "%zd",max_ind)
        printf("%s,%s,%s\n", buffer, buffer2, buffer3)
