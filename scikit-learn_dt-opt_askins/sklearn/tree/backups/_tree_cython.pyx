...
from _utils_cython cimport safe_realloc
from _utils_cython cimport sizet_ptr_to_ndarray
...
cdef cnp.float64_t EPSILON = np.finfo('double').eps
...
TREE_LEAF = -1
TREE_UNDEFINED = -2
cdef cnp.intp_t _TREE_LEAF = TREE_LEAF
cdef cnp.intp_t _TREE_UNDEFINED = TREE_UNDEFINED
...
cdef class TreeBuilder:
    ...
    cpdef build(
        ...
        const cnp.float64_t[:] sample_weight=None,
        const unsigned char[::1] missing_values_in_feature_mask=None,
        cnp.int32_t[:] sensor_types=None,
        cnp.int32_t[:] depth_types=None,
        cnp.int32_t[:] time_types=None,
    ):
...
cdef class DepthFirstTreeBuilder(TreeBuilder):
    ...
    def __cinit__(self, Splitter splitter, cnp.intp_t min_samples_split,
                  cnp.intp_t min_samples_leaf, cnp.float64_t min_weight_leaf,
                  cnp.intp_t max_depth, cnp.float64_t min_impurity_decrease):
    ...
    cpdef build(
        ...
        # Initial capacity
        cdef cnp.intp_t init_capacity

        if tree.max_depth <= 10:
            init_capacity = <cnp.intp_t> (2 ** (tree.max_depth + 1)) - 1
        else:
            init_capacity = 2047
        ...
        cdef cnp.intp_t max_depth = self.max_depth
        cdef cnp.intp_t min_samples_leaf = self.min_samples_leaf
        cdef cnp.float64_t min_weight_leaf = self.min_weight_leaf
        cdef cnp.intp_t min_samples_split = self.min_samples_split
        cdef cnp.float64_t min_impurity_decrease = self.min_impurity_decrease
        ...
        cdef cnp.intp_t end
        cdef cnp.intp_t depth
        cdef cnp.intp_t parent
        cdef bint is_left
        cdef cnp.intp_t n_node_samples = splitter.n_samples
        cdef cnp.float64_t weighted_n_node_samples
        cdef SplitRecord split
        cdef cnp.intp_t node_id

        cdef cnp.float64_t impurity = INFINITY
        cdef cnp.float64_t lower_bound
        cdef cnp.float64_t upper_bound
        cdef cnp.float64_t middle_value
        cdef cnp.float64_t left_child_min
        cdef cnp.float64_t left_child_max
        cdef cnp.float64_t right_child_min
        cdef cnp.float64_t right_child_max
        cdef cnp.intp_t n_constant_features
        cdef bint is_leaf
        cdef bint first = 1
        cdef cnp.intp_t max_depth_seen = -1
        cdef int rc = 0
...
cdef class BestFirstTreeBuilder(TreeBuilder):
    ...
    """
    cdef cnp.intp_t max_leaf_nodes

    def __cinit__(self, Splitter splitter, cnp.intp_t min_samples_split,
                  cnp.intp_t min_samples_leaf,  min_weight_leaf,
                  cnp.intp_t max_depth, cnp.intp_t max_leaf_nodes,
                  cnp.float64_t min_impurity_decrease):
        self.splitter = splitter
        self.min_samples_split = min_samples_split
        self.min_samples_leaf = min_samples_leaf
        self.min_weight_leaf = min_weight_leaf
    ...
    cpdef build(
        ...
        # Parameters
        cdef Splitter splitter = self.splitter
        cdef cnp.intp_t max_leaf_nodes = self.max_leaf_nodes
        ...
        cdef cnp.intp_t n_node_samples = splitter.n_samples
        cdef cnp.intp_t max_split_nodes = max_leaf_nodes - 1
        cdef bint is_leaf
        cdef cnp.intp_t max_depth_seen = -1
        cdef int rc = 0
        cdef Node* node
        ...
        # Initial capacity
        cdef cnp.intp_t init_capacity = max_split_nodes + max_leaf_nodes
        tree._resize(init_capacity)
    ...
    cdef inline int _add_split_node(
        ...
        cdef SplitRecord split
        cdef cnp.intp_t node_id
        cdef cnp.intp_t n_node_samples
        cdef cnp.intp_t n_constant_features = 0
        cdef cnp.float64_t min_impurity_decrease = self.min_impurity_decrease
        cdef cnp.float64_t weighted_n_node_samples
        cdef bint is_leaf
...
cdef class Tree:
    ...
    max_depth : cnp.intp_t
        The depth of the tree, i.e. the maximum depth of its leaves.

    children_left : array of cnp.intp_t, shape [node_count]
        children_left[i] holds the node id of the left child of node i.
        For leaves, children_left[i] == TREE_LEAF. Otherwise,
        children_left[i] > i. This child handles the case where
        X[:, feature[i]] <= threshold[i].

    children_right : array of cnp.intp_t, shape [node_count]
    ...
    feature : array of cnp.intp_t, shape [node_count]
        feature[i] holds the feature to split on, for the internal node i.

    threshold : array of cnp.float64_t, shape [node_count]
        threshold[i] holds the threshold for the internal node i.

    value : array of cnp.float64_t, shape [node_count, n_outputs, max_n_classes]
        Contains the constant prediction value of each node.

    impurity : array of cnp.float64_t, shape [node_count]
        impurity[i] holds the impurity (i.e., the value of the splitting
        criterion) at node i.

    n_node_samples : array of cnp.intp_t, shape [node_count]
        n_node_samples[i] holds the number of training samples reaching node i.

    weighted_n_node_samples : array of cnp.float64_t, shape [node_count]
    ...
    @property
    def cost(self):
        return self._get_node_ndarray()['cost'][:self.node_count]

    @property
    def cost_objective(self):
        return self._get_node_ndarray()['cost_objective'][:self.node_count]
    ...
    def __cinit__(self, cnp.intp_t n_features, cnp.ndarray n_classes, cnp.intp_t n_outputs):
        """Constructor."""
        cdef cnp.intp_t dummy = 0
    ...
    def __setstate__(self, d):
        ...
        memcpy(self.value, cnp.PyArray_DATA(value_ndarray),
               self.capacity * self.value_stride * sizeof(cnp.float64_t))
    ...
    cdef int _resize_c(self, cnp.intp_t capacity=INTPTR_MAX) except -1 nogil:
        """Guts of _resize
    ...
    cdef cnp.intp_t _add_node(self, cnp.intp_t parent, bint is_left, bint is_leaf,
        ...
        """
        cdef cnp.intp_t node_id = self.node_count
        ...
        node.cost = cost
        node.cost_objective = cost_objective
    ...
    cdef inline cnp.ndarray _apply_dense(self, object X):
        ...
        cdef cnp.float32_t X_i_node_feature

        # Initialize output
        cdef cnp.intp_t[:] out = np.zeros(n_samples, dtype=np.intp)
        ...
        with nogil:
            for i in range(n_samples):
                ...
                out[i] = <cnp.intp_t>(node - self.nodes)  # node offset
    ...
    cdef inline cnp.ndarray _apply_sparse_csr(self, object X):
        ...
        # Extract input
        cdef const cnp.float32_t[:] X_data = X.data
        cdef const cnp.int32_t[:] X_indices = X.indices
        cdef const cnp.int32_t[:] X_indptr = X.indptr
        ...
        # Initialize auxiliary data-structure
        cdef cnp.float32_t feature_value = 0.
        cdef Node* node = NULL
        cdef cnp.float32_t* X_sample = NULL
        cdef cnp.intp_t i = 0
        cdef cnp.int32_t k = 0

        # feature_to_sample as a data structure records the last seen sample
        # for each feature; functionally, it is an efficient way to identify
        # which features are nonzero in the present sample.
        cdef cnp.intp_t* feature_to_sample = NULL

        safe_realloc(&X_sample, n_features)
        safe_realloc(&feature_to_sample, n_features)

        with nogil:
            memset(feature_to_sample, -1, n_features * sizeof(cnp.intp_t))
    ...
    cdef inline object _decision_path_dense(self, object X):
        ...
        # Initialize output
        cdef cnp.intp_t[:] indptr = np.zeros(n_samples + 1, dtype=np.intp)
        cdef cnp.intp_t[:] indices = np.zeros(
        ...
        with nogil:
            for i in range(n_samples):
                ...
                while node.left_child != _TREE_LEAF:
                    # ... and node.right_child != _TREE_LEAF:
                    indices[indptr[i + 1]] = <cnp.intp_t>(node - self.nodes)
                    indptr[i + 1] += 1
        ...
        cdef cnp.intp_t[:] data = np.ones(shape=len(indices), dtype=np.intp)
    ...
    cpdef compute_feature_importances(self, normalize=True):
        ...
        cdef cnp.float64_t normalizer = 0.
    ...
    def compute_partial_dependence(self, cnp.float32_t[:, ::1] X,
                                   const cnp.intp_t[::1] target_features,
                                   cnp.float64_t[::1] out):
        ...
        """
        cdef:
            cnp.float64_t[::1] weight_stack = np.zeros(self.node_count,
                                                   dtype=np.float64)
            cnp.intp_t[::1] node_idx_stack = np.zeros(self.node_count,
                                                  dtype=np.intp)
            cnp.intp_t sample_idx
            cnp.intp_t feature_idx
            cnp.intp_t stack_size
            cnp.float64_t left_sample_frac
            cnp.float64_t current_weight
            cnp.float64_t total_weight  # used for sanity check only
            Node *current_node  # use a pointer to avoid copying attributes
            cnp.intp_t current_node_idx
            bint is_target_feature
            cnp.intp_t _TREE_LEAF = TREE_LEAF  # to avoid python interactions
...
def _dtype_dict_with_modified_bitness(dtype_dict):
    # field names in Node struct with cnp.intp_t types (see sklearn/tree/_tree.pxd)
...
def _all_compatible_dtype_dicts(dtype):
    # The Cython code for decision trees uses platform-specific cnp.intp_t
...
cdef class _CCPPruneController:
    ...
    to control pruning.
    """
    cdef bint stop_pruning(self, cnp.float64_t effective_alpha) noexcept nogil:
        """Return 1 to stop pruning and 0 to continue pruning"""
        return 0

    cdef void save_metrics(self, cnp.float64_t effective_alpha,
                           cnp.float64_t subtree_impurities) noexcept nogil:
...
cdef class _AlphaPruner(_CCPPruneController):
    ...
    cdef cnp.float64_t ccp_alpha
    cdef cnp.intp_t capacity

    def __cinit__(self, cnp.float64_t ccp_alpha):
...
cdef class _PathFinder(_CCPPruneController):
    ...
    cdef cnp.float64_t[:] ccp_alphas
    cdef cnp.float64_t[:] impurities
    cdef cnp.uint32_t count

    def __cinit__(self,  cnp.intp_t node_count):
...
cdef _cost_complexity_prune(unsigned char[:] leaves_in_subtree,  # OUT
    ...
    cdef:
        cnp.intp_t i
        cnp.intp_t n_nodes = orig_tree.node_count
        # prior probability using weighted samples
        cnp.float64_t[:] weighted_n_node_samples = orig_tree.weighted_n_node_samples
        cnp.float64_t total_sum_weights = weighted_n_node_samples[0]
        cnp.float64_t[:] impurity = orig_tree.impurity
        # weighted impurity of each node
        cnp.float64_t[:] r_node = np.empty(shape=n_nodes, dtype=np.float64)

        cnp.intp_t[:] child_l = orig_tree.children_left
        cnp.intp_t[:] child_r = orig_tree.children_right
        cnp.intp_t[:] parent = np.zeros(shape=n_nodes, dtype=np.intp)

        stack[CostComplexityPruningRecord] ccp_stack
        CostComplexityPruningRecord stack_record
        cnp.intp_t node_idx
        stack[cnp.intp_t] node_indices_stack

        cnp.intp_t[:] n_leaves = np.zeros(shape=n_nodes, dtype=np.intp)
        cnp.float64_t[:] r_branch = np.zeros(shape=n_nodes, dtype=np.float64)
        cnp.float64_t current_r
        cnp.intp_t leaf_idx
        cnp.intp_t parent_idx
        ...
        cnp.intp_t pruned_branch_node_idx
        cnp.float64_t subtree_alpha
        cnp.float64_t effective_alpha
        cnp.intp_t n_pruned_leaves
        cnp.float64_t r_diff
        cnp.float64_t max_float64 = np.finfo(np.float64).max
...
def _build_pruned_tree_ccp(
    ...
    orig_tree : Tree
        Original tree
    ccp_alpha : positive cnp.float64_t
...
def ccp_pruning_path(Tree orig_tree):
    ...
    cdef:
        cnp.uint32_t total_items = path_finder.count
        cnp.float64_t[:] ccp_alphas = np.empty(shape=total_items, dtype=np.float64)
        cnp.float64_t[:] impurities = np.empty(shape=total_items, dtype=np.float64)
        cnp.uint32_t count = 0
...
cdef _build_pruned_tree(
    ...
    cdef:
        cnp.intp_t orig_node_id
        cnp.intp_t new_node_id
        cnp.intp_t depth
        cnp.intp_t parent
        bint is_left
        bint is_leaf
        ...
        cnp.intp_t value_stride = orig_tree.value_stride
        cnp.intp_t max_depth_seen = -1
        int rc = 0
        Node* node
        cnp.float64_t* orig_value_ptr
        cnp.float64_t* new_value_ptr
    ...
    with nogil:
        ...
        while not prune_stack.empty():
            ...
            new_node_id = tree._add_node(
                ...
                node.cost, node.cost_objective, node.improvement)
            ...
            memcpy(new_value_ptr, orig_value_ptr, sizeof(cnp.float64_t) * value_stride)
