...
# from libc.stdlib cimport rand_r
...
# from ..utils._random cimport rand_r
...
#     unsigned int rand_r(unsigned int* seed) nogil
...
cdef inline cnp.ndarray sizet_ptr_to_ndarray(cnp.intp_t* data, cnp.intp_t size):
...
cdef inline cnp.intp_t rand_int(cnp.intp_t low, cnp.intp_t high,
                            cnp.uint32_t* random_state) noexcept nogil:
    """Generate a random integer in [low; end)."""
    return low + our_rand_r(random_state) % (high - low)


cdef inline cnp.float64_t rand_uniform(cnp.float64_t low, cnp.float64_t high,
                                   cnp.uint32_t* random_state) noexcept nogil:
    """Generate a random cnp.float64_t in [low; high)."""
    return ((high - low) * <cnp.float64_t> our_rand_r(random_state) /
            <cnp.float64_t> RAND_R_MAX) + low


cdef inline cnp.float64_t log(cnp.float64_t x) noexcept nogil:
    return ln(x) / ln(2.0)
...
cdef class WeightedPQueue:
    ...
    def __cinit__(self, cnp.intp_t capacity):
    ...
    cdef int push(self, cnp.float64_t data, cnp.float64_t weight) except -1 nogil:
        ...
        or 0 otherwise.
        """
        cdef cnp.intp_t array_ptr = self.array_ptr
    ...
    cdef int remove(self, cnp.float64_t data, cnp.float64_t weight) noexcept nogil:
        ...
        cdef cnp.intp_t idx_to_remove = -1
        cdef cnp.intp_t i
...
cdef class WeightedMedianCalculator:
    ...
    def __cinit__(self, cnp.intp_t initial_capacity):
    ...
    cdef int push(self, cnp.float64_t data, cnp.float64_t weight) except -1 nogil:
        ...
        cdef cnp.float64_t original_median = 0.0
...
def _any_isnan_axis0(const cnp.float32_t[:, :] X):
    """Same as np.any(np.isnan(X), axis=0)"""
    cdef:
        cnp.intp_t i, j
        cnp.intp_t n_samples = X.shape[0]
        cnp.intp_t n_features = X.shape[1]
