...
class TLEVEL:
    ...
    def readLines(self):
        ...
        infile = open(str(self.directory / self.dirList[self.OUT_index]),"r")
...
class NODINF:
    ...
    def readData(self):
        ...
        if not self.lbinary:
            ...
            infile = open(str(self.directory / self.dirList[self.OUT_index]),"rb")
            # self.lines = infile.readlines()
            # # print(self.lines[0])
...
##        dayIndex = self.calcDayIndex(day)
    ...
    def writeData(self):
        ...
        outfile = open(str(self.directory / self.dirList[self.OUT_index]),"w")
        outfile.seek(0,0)                    
...
class NODINFFAST:
    ...
    def readData(self):
        ...
        flag = True
        with open(str(self.directory / self.dirList[self.OUT_index]),"r") as f:
            ...
            while line:
                if 'Time:' in line[0]:
                    ...
                    while sline[0] != 'end':
                        headdata.append(float(sline[2]))
                        wcdata.append(float(sline[3]))
                        fluxdata.append(float(sline[6]))
                        sline = list(islice(f,1))[0].split()
                    wcvalues.append(wcdata)
                    headvalues.append(headdata)
                    fluxvalues.append(fluxdata)
                    # values[time] = data
                line = list(islice(f,1))

        self.times = times

        return np.array(wcvalues), np.array(headvalues), np.array(fluxvalues)
